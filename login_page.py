...
from text_to_audio import footer_notes
...
def logged_in():
    ...
    if not st.session_state.is_logged_in:
        register_message = "Not registered yet? Join us today and start transcribing with ease enjoying cheap and affordable transcription service <b style='color:black;'> @ ksh. 6/per audio minute </b> coming with 2 free minutes, completely free economy plan and  other free services like text analysis and multiple translation languages available!</b>"
        register_css = f"""
        ...
        register_css = f"""
                    <div style="width:100%; overflow:auto; position:relative; min-height:50px; background:linear-gradient(to top right, rgba(255,0,100,220.4), rgba(0,0,0,0.7),rgba(100,150,250,0.6), rgba(0,0,255,0.5)); padding:20px; border-radius:8px; 
                        ...
                        <p style="color:#F7F9F9; position:absolute; font-size:20px; font-weight:bolder; left:30%; right:30%; margin: 0;">{register_message}</p>
