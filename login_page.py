...
def logged_in():
    ...
    if not st.session_state.is_logged_in:
        ...
        register_css = f"""
            <div style="width:100%; overflow:scroll; position:relative; min-height:100px; background:linear-gradient(to top right, rgba(255,0,0,0.4), rgba(0,255,0,0.5),rgba(100,150,250,0.6), rgba(0,0,255,0.5)); padding:20px; border-radius:8px; 
        ...
        if new_username:
            ...
            if full_name and email:
                ...
                st.markdown(
                    f"""
                    <div style="background-color: #34495E; padding: 15px; overflow:scroll; border-radius: 8px; margin-bottom: 20px;">
                ...
                with st.expander("View comments"):
                    ...
                    for index, com in enumerate(results):
                        st.markdown(f"""
                                    <div style="background-color:black; width:100%; height:200px; position:relative"; overflow:scroll;>
                                        <h4 style="color:red; position:absolute; left:2%; top:30%; font-family:sans-serif; text-transform:capitalize;">{index + 1}. {com[2]} &nbsp;<span style="color:blue;font-size:12px;">time: {com[4]}</span></h4>
